        ...
        <div class="row">
            <div class="col-md-8">
                ...
                <div class="post-container">
                    ...
                    if(isset($_GET['aid'])){
                    $aid_auth= $_GET['aid'];
                    //echo $aid_auth;//for testing
                    //die();
                }
                    $sqlp= "SELECT *FROM post JOIN user ON post.author=user.user_id WHERE post.author ={$aid_auth}";
                    // echo $sqlp; //for testing
                    // die();
                    $queryp=mysqli_query($conn,$sqlp) or die("Query is failed");
                    $row1=mysqli_fetch_assoc($queryp);
                    //print_r($row1); for testing data is coming or not from database
                    ?>
                    <h2 class="page-heading"><?php echo" Author: ".$row1['first_name']." ".$row1['last_name'];?></h2>
                        <?php
                    $limit=3;
                    //$page = $_GET['page'];
                    //echo $page;
                    ...
                    $sqls="SELECT post.post_id, post.title, post.description,post.author, post.post_date,post_img, category.category_name,user.first_name,user.last_name,post.category FROM post 
                    LEFT JOIN category on post.category= category.category_id
                    LEFT JOIN user ON post. author = user.user_id
                    WHERE post.author={$aid_auth}
                ...
                echo" <h2> Sorry Record not Found..<h2>";
                    ...
                    if(mysqli_num_rows($queryp) > 0){
                      ...
                      if($page >1){
                              echo'<li ><a href="index.php?aid='.$aid_auth .'&page='.($page-1).'">Prev</a></li>';
                            }
                      for($j=1;$j<=$total_page;$j++){
                           ...
                           echo "<li class='$active'><a href='index.php?aid='.$aid_auth .'&page={$j}'>{$j}</a></li>";

                      }
                       if($total_page>$page){
                        echo'<li ><a href="index.php?aid='.$aid_auth .'&page='.($page+1).'">Next</a></li>';
                      }
